...
class SurfMono(Component):
    ...
    def setSLD(self):
        ...
        if self.tailVolume == 0:
            tailSLDs['contrast0'] = Parameter(self.guessSLD(), 'tail_layer_contrast0',
            ...
            tailSLDs['contrast0'] = Parameter(a,'tail_layer_contrast0', bounds=(a - (0.25 * a), a + (0.25 * a)),
                                                                                      vary=True)
        headSLDs['contrast0'] = Parameter(1., 'head_layer_contrast0')
        for i in range(1, self.numberofcontrasts):
            tailSLDs['contrast%s' % i] = Parameter(1, 'tail_layer_contrast%s' % i)
            headSLDs['contrast%s' % i] = Parameter(1, 'head_layer_contrast%s' % i)
            subSLDs['contrast%s' % i] = Parameter(self.subPhaseSLD['contrast%s' % i], 'sub%s' % i)
            superSLDs['contrast%s' % i] = Parameter(self.superPhaseSLD['contrast%s' % i], 'super%s' % i)
        for i in range(0, self.numberofcontrasts):
            self.tails['contrast%s' % i] = SLD(tailSLDs['contrast%s' % i], name='tail_contrast%s' % i)
            self.heads['contrast%s' % i] = SLD(headSLDs['contrast%s' % i], name='head_contrast%s' % i)
    ...
    def setConstraints(self):
        ...
        if self.numberofcontrasts == 1:
            self.head_layers['contrast0'].sld.real.constraint = \
                ...
                (self.head_thick * self.tailScatLen['contrast0'] * (1 - self.head_layers['contrast0'].vfsolv))
        if self.numberofcontrasts == 2:
            ...
            self.tail_layers['contrast1'].sld.real.constraint = \
                (self.head_layers['contrast1'].sld.real * self.head_thick * self.tailScatLen['contrast1'] *
                 (1 - self.head_layers['contrast1'].vfsolv)) / (self.tail_thick *self.headScatLen['contrast1'])
        if self.numberofcontrasts > 2:
            if self.numberofcontrasts % 2 != 0:
                for i in range(0, self.numberofcontrasts - 2, 2):
                    ...
                    self.head_layers['contrast%s' % a].sld.real.constraint = \
                        (self.tail_layers['contrast%s' % a].sld.real * self.tail_thick *
                         self.headScatLen['contrast%s' % a]) / (self.head_thick * self.tailScatLen['contrast%s' % a] *
                                                                (1 - self.head_layers['contrast%s' % a].vfsolv))
                    ...
                    self.tail_layers['contrast%s' % b].sld.real.constraint = \
                        (self.head_layers['contrast%s' % b].sld.real * self.head_thick *
                         self.tailScatLen['contrast%s' % b] * (1 - self.head_layers['contrast%s' % b].vfsolv)) / \
                ...
                self.head_layers['contrast%s' % str(self.numberofcontrasts-1)].sld.real.constraint = \
                    ...
                    (self.head_thick * self.tailScatLen['contrast%s' % str(self.numberofcontrasts-1)] *
                     (1 - self.head_layers['contrast%s' % str(self.numberofcontrasts-1)].vfsolv))
            else:
                for i in range(0, self.numberofcontrasts - 2, 2):
                    a = str(i)
                    b = str(i + 1)
                ...
                self.head_layers['contrast%s' % str(self.numberofcontrasts-2)].sld.real.constraint = \
                    ...
                    (self.head_thick * self.tailScatLen['contrast%s' % str(self.numberofcontrasts-2)] *
                     (1 - self.head_layers['contrast%s' % str(self.numberofcontrasts-2)].vfsolv))
                self.head_layers['contrast%s' % str(self.numberofcontrasts-1)].sld.real.constraint = \
